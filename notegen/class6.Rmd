...
p1 <- chen[,sum(useCnt),by=(chennel)] %>% ggplot(aes(chennel, V1)) + geom_bar(stat = "identity")
p2 <- ggplot(comp[,.N,by=(useDate)], aes(x=useDate, y=N)) + geom_line() + scale_x_date(date_breaks="6 months")
p3 <- ggplot(comp[,.N,by=.(useDate,partner)], aes(x=useDate, y=N, color=partner)) + geom_line() + scale_x_date(date_breaks="6 months") + theme(legend.position = "top")
...
.pull-right[
```{r warning=F, message=F, echo=F, fig.height=6}
library(gridExtra)
library(tidyverse)
wifi<-fread("../data/wifi.csv", encoding="UTF-8")
...
p1<-pg + geom_point(data=sw, aes(x=lon, y=lat, size=N), color="blue", alpha=0.3)
...
p2<-pg + geom_point(data=sw, aes(x=lon, y=lat, size=N), color="blue", alpha=0.3)
