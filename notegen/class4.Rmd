...
boundary : 글자 경계의 종류를 결정 c("character", "line_break","sentence","word")    
...
1. recomen의 item.csv 파일을 불러와주세요.
    - cate_3_name 이 "립"으로 시작하는 제품은 어느 파트너사에서 판매하나요?
    - cate_3_name 이 "소스"로 끝나는 제품은 무엇이 있나요?
    - "/" 로 합쳐져 있는 cate_3_name를 모두 잘라서 유일한 제품명만 세면 모두 몇개 인가요?

2. 아래 조건에 해당하는 정규표현식 패턴을 작성해주세요.
    - 김경수, 김찬수, 김갑수 중 하나 이상 포함되어 있는지 확인
    - 
    - 
